...
```bash
cp .env.example .env
# set POSTGRES_PASSWORD and BETTER_AUTH_SECRET (openssl rand -hex 32 each),
# ADMIN_EMAIL, and ADMIN_PASSWORD (8+ chars, letters + numbers)
...
`BETTER_AUTH_SECRET` is required (no default); generate with `openssl rand -hex 32`.
The app refuses to start if the secret is missing or weak (must be at least 32
...
| `BETTER_AUTH_SECRET` | Auth signing secret (required; strong generated secret) |
